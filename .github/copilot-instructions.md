...
```
> There are no actual test files in the repo yet. Package `test` scripts are stubs (`echo stub`).
...
- **`apps/cloud`** — Web application (Next.js 15, port `:3002`)
  - Currently a stub. `next.config.mjs` sets `MCH_TARGET: 'cloud'`.
  - Supabase client dependencies (`@supabase/supabase-js`, `@supabase/ssr`) are installed for future auth/workspaces.
