...
export default function ChatArea() {
  ...
  return (
    <section className="flex-1 bg-white flex flex-col h-full overflow-hidden" aria-label="Área de chat">
      <div className="px-4 py-3 border-b border-zinc-200 bg-zinc-50 flex items-center justify-between gap-3 shadow-sm">
        <h2 className="font-semibold text-zinc-900 truncate">
          # <span className="font-mono">{currentChannel}</span>
        </h2>
        <div className="flex items-center gap-2 text-xs">
          <span className="text-zinc-600 hidden sm:inline">Pensamento:</span>
          <Button
      ...
      <div className="flex-1 px-4 sm:px-6 py-4 overflow-y-auto space-y-5" role="log" aria-live="polite">
        {channelMessages.length === 0 ? (
          <div className="text-center text-zinc-400 italic mt-12">
              ...
              <div
                className={`max-w-[85%] sm:max-w-[75%] p-3 sm:p-4 rounded-xl shadow-sm leading-relaxed ${
                  msg.sender === 'User'
                    ...
                    : msg.sender === 'Sistema'
                    ? 'bg-zinc-100 text-zinc-800 border border-zinc-200 text-sm italic'
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-zinc-100 border border-zinc-200 rounded-xl px-4 py-3 text-sm italic text-zinc-600 inline-flex items-center gap-2" aria-live="polite">
      ...
      <div className="p-3 sm:p-4 bg-zinc-50 border-t border-zinc-200 shadow-inner">
        <div className="flex gap-1 sm:gap-2 items-center bg-white p-1.5 rounded-xl border border-zinc-300 focus-within:ring-2 ring-indigo-500/20 transition-all shadow-sm">
          <input
            ref={fileInputRef}
            type="file"
            accept=".csv,.json"
          ...
          <input
            ...
            className="flex-1 px-2 py-2 outline-none bg-transparent min-w-0"
